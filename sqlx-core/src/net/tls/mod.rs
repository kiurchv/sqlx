...
{
    #[cfg(feature = "_tls-cfw")]
    return Ok(with_socket.with_socket(tls_cfw::handshake(socket, config).await?));
    ...
    )))]
    {
        ...
        panic!("one of the `runtime-*-cfw-tls`, `runtime-*-native-tls` or `runtime-*-rustls` features must be enabled")
    }
}
